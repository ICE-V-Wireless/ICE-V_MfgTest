...
esp_err_t adc_c3_init(void)
{
    ...
    ESP_ERROR_CHECK_WITHOUT_ABORT((ret = adc1_config_width(ADC_WIDTH_BIT_DEFAULT)));
    if(ret) cnt++;
	ESP_ERROR_CHECK_WITHOUT_ABORT((ret = adc1_config_channel_atten(ADC_C3_CHL, ADC_EXAMPLE_ATTEN)));
    if(ret) cnt++;
